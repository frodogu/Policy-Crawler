...
def getURL(url, tries_num=5, sleep_time=0, time_out=10, max_retry=5):
    ...
    proxy = {
        "http": "http://110.37.84.147:8080",
        "https": "http://110.37.84.147:8080",
    }  # 这里需要替换成可用的代理IP
    header = randHeader()
    ...
    try:
        ...
        if tries_num_p > 0:
            ...
            print(
                getCurrentTime(),
                url,
                "URL Connection Error: 第",
                max_retry - tries_num_p,
                "次 Retry Connection",
                e,
            )
...
# def start_requests():
#     url = "http://hrss.sz.gov.cn/szsi/sbjxxgk/zcfggfxwj/zcfg/index.html"
#     html = get_url(url)
#     return html
